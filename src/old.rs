...
use snafu::{ErrorCompat, Snafu, Whatever};
pub use winit;
...
/// Generic Error type for reporting errors to the engine
#[derive(Debug)]
pub struct Report {
    source: Box<dyn assets::formats::FormatError>,
}

impl<E> From<E> for Report
where
    E: assets::formats::FormatError + 'static,
{
...
impl fmt::Display for Report {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.source.iter_chain().skip(1).next().is_some() {
            writeln!(f, "\nCaused by:")?;
        }

        for (n, err) in self.source.iter_chain().skip(1).enumerate() {
            writeln!(f, "\t{n}: {err}")?;
        }
...
pub trait App: Sized {
    ...
    fn init(_ctx: &mut Context) -> Result<Self, Report>;
}
...
pub fn run<A: App + 'static>() -> ! {
    ...
    let mut app = match A::init(&mut scene) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("{e:#}");
            std::process::exit(-1);
